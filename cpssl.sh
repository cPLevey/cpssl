...
#
	...
	_request=($(python -m json.tool < /var/cpanel/autossl_queue_cpanel.json|grep -oP '(?<="request_time": ")[^"]+'|sed 's/T/ /; s/Z//'));
	...
	function _resolve () {
		...
		if [ "${#_resolv[@]}" -eq 0 ]; then
			...
			for _ip in "${_resolv[@]}"; do
				if [[ "${_ips[@]}" =~ "$_ip" ]]; then
					local _resolve_res='1';
					printf "%b" "$COL_GREEN";
					printf "(local %s): " "$_ip";
					printf "%b" "$COL_RESET";
				fi;
			done;
			if [ "${_resolve_res:=null}" == null ]; then
				printf "%b" "$COL_RED";
				printf "(remote %s): " "$_ip";
				printf "%b" "$COL_RESET";
			fi;
	...
	for ((i=0; i<${#_domains[@]}; i++)); do
		...
		_reqepoch="$(date -d "${_request[$i]}" +%s)";
		_curepoch="$(date +%s)";
		_diff="$((_curepoch-_reqepoch))";
		_count="$((604800-_diff))";
		_count_day="$((_diff/3600/24))";
		_count_hr="$((_diff/3600%24))";
		if [ "$_count" -lt 86400 ]; then
			_expired+=("${_domains[$i]}::${_orders[$i]}");
			printf "Expired:%b" "$COL_RED";
			printf " %s days" "$_count_hr"; printf " %s hrs...\n" "$_count_hr";
			printf "%b" "$COL_RESET";
